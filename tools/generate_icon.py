...
on a black square, at 85.4pt on a 100pt canvas, optically centred on the ink
rather than the line box.

LightFog only needs a PNG because it's a plain Android app with a launcher icon.
A light-sdk tool has no launcher icon to set: lighttool.toml has no icon field
and the generated manifest sets no android:icon. Its one icon slot is
windowSplashScreenAnimatedIcon, which sdk/client's theme points at
@drawable/loading_text_icon — so this also emits a vector drawable of the same
name, which the application module's copy overrides.

    python3 tools/generate_icon.py                    # uses the bundled font
    python3 tools/generate_icon.py path/to/Font.ttf   # or bring your own
...
# Committed so the mark is reproducible without hunting for the font, the same
# reason LightFog keeps its copy in assets/fonts. Public Sans is SIL OFL 1.1.
DEFAULT_FONT = REPO / "assets" / "fonts" / "PublicSans-Regular.ttf"
...
def main() -> None:
    if len(sys.argv) > 2:
        sys.exit(__doc__)
    font_path = Path(sys.argv[1]) if len(sys.argv) == 2 else DEFAULT_FONT
